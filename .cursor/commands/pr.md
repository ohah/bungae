...
- **On update**:
  - First, check the PR state: `gh pr view <PR-number> --json state -q .state`.
...
This repo (ohah/bungae) uses the **ohah** GitHub account for push and PR.
...
- **SSH remote**: So that `git push` authenticates as ohah, origin must use the SSH host `github.com-private` (see `~/.ssh/config`). Before any push, ensure origin is set to:
  ```bash
  git remote set-url origin git@github.com-private:ohah/bungae.git
  ```
...
2. **SSH remote**: Ensure origin is `git@github.com-private:ohah/bungae.git` so push uses ohah's SSH key; if not, run `git remote set-url origin git@github.com-private:ohah/bungae.git`.
