...
This repo (ohah/bungae) uses the **ohah** GitHub account for posting reviews.
...
5. **Submit the review (summary + inlines together when there are inlines)**
   - **5-a. When there is at least one inline suggestion**
     ...
     - Commands:
       ...
       gh api repos/ohah/bungae/pulls/$(gh pr view --json number -q .number)/reviews --input review-payload.json
       ```
...
- Run from the repo root with `gh` authenticated. This repo (ohah/bungae): use ohah for posting reviews; switch gh before submit and restore after (see "gh account for this repo" and step 1, step 6).
