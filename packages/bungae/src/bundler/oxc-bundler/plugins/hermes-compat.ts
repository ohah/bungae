    ...
    renderChunk: {
      async handler(code, _chunk) {
        try {
          ...
          // DEBUG: dump raw bundle before SWC
          try {
            writeFileSync('/tmp/bungae-pre-swc.js', code, 'utf-8');
          } catch {}
          ...
          // DEBUG: dump bundle after SWC
          try {
            writeFileSync('/tmp/bungae-post-swc.js', patched, 'utf-8');
          } catch {}
          ...
          console.warn(
            `[hermes-compat] SWC transform failed: ${error.message}`,
          );
          return null;
        }
      },
